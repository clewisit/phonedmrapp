...
public class SpringScroller {
    ...
    public boolean computeScrollOffset() {
        ...
        if (i != 0) {
            ...
            float min = Math.min(((float) (currentAnimationTimeMillis - this.mStartTime)) / 1000.0f, (float) MAX_DELTA_TIME);
            if (min != 0.0f) {
                f = min;
            }
            ...
            if (this.mOrientation == 2) {
                ...
                double d3 = this.mStartY + (f * updateVelocity);
                this.mCurrY = d3;
                ...
                double d4 = this.mStartX + (f * updateVelocity2);
                this.mCurrX = d4;
    ...
    public boolean isAtEquilibrium(double d, double d2, double d3) {
        if (d2 >= d3 || d <= d3) {
            ...
            if (compare <= 0 || d >= d3) {
                return (compare == 0 && Math.signum(this.mOriginVelocity) != Math.signum(d)) || Math.abs(d - d3) < 1.0d;
            }
            return true;
        }
        return true;
