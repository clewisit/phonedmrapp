...
public class UtilContactsData {
    ...
    public long addContact() {
        ...
        return this.mDB.insert(this.mDBName, null, contentValues);
    }
    ...
    public ArrayList<ContactData> getAllContacts(int i) {
        ...
        this.mDB = this.mHelper.getWritableDatabase();
        ArrayList<ContactData> arrayList = new ArrayList<>();
        Cursor query = this.mDB.query(this.mDBName, null, "contact_type=?", new String[]{i + ""}, null, null, null);
        while (query.moveToNext()) {
    ...
    public ContactData getContact(int i, int i2) {
        ...
        this.mDB = writableDatabase;
        String str = this.mDBName;
        Cursor query = writableDatabase.query(str, null, "contact_type=? AND contact_number=?", new String[]{i + "", i2 + ""}, null, null, null, "1");
        Bitmap bitmap = null;
        if (query.moveToNext()) {
    ...
    public ContactData getContact(int i) {
        ...
        this.mDB = writableDatabase;
        String str = this.mDBName;
        Cursor query = writableDatabase.query(str, null, InboundSmsHandler.SELECT_BY_ID, new String[]{i + ""}, null, null, null, "1");
        Bitmap bitmap = null;
        if (query.moveToNext()) {
            int i2 = query.getInt(query.getColumnIndex("_id"));
            int i3 = query.getInt(query.getColumnIndex(TABLE_TYPE));
    ...
    public ContactData getActiveContact() {
        SQLiteDatabase writableDatabase = this.mHelper.getWritableDatabase();
        this.mDB = writableDatabase;
        Cursor query = writableDatabase.query(this.mDBName, null, "contact_active=?", new String[]{"1"}, null, null, null, "1");
        Bitmap bitmap = null;
        if (query.moveToNext()) {
            int i = query.getInt(query.getColumnIndex("_id"));
            int i2 = query.getInt(query.getColumnIndex(TABLE_TYPE));
    ...
    public void resetData() {
        SQLiteDatabase writableDatabase = this.mHelper.getWritableDatabase();
        this.mDB = writableDatabase;
        writableDatabase.delete(this.mDBName, null, null);
    }
