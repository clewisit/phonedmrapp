...
public final class MessageDispatcher implements PacketReceiver {
    ...
    @Override // com.pri.prizeinterphone.serial.PacketReceiver
    public void onReceive(final Packet packet, SerialPort serialPort) {
        final MessageHandler messageHandler = this.handlers.get(Byte.valueOf(packet.getCmd()));
        Log.i(TAG, "onReceive: handler " + messageHandler);
        if (messageHandler != null) {
            this.executor.execute(new Runnable() { // from class: com.pri.prizeinterphone.serial.MessageDispatcher$$ExternalSyntheticLambda0
