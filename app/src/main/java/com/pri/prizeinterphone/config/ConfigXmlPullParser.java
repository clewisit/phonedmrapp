...
public class ConfigXmlPullParser {
    ...
    public static class SingletonHolder {
        private static final ConfigXmlPullParser INSTANCE = new ConfigXmlPullParser(PrizeInterPhoneApp.getContext());
    ...
    public ParseConfig parse() {
        ...
        try {
            ...
            for (int eventType = newPullParser.getEventType(); eventType != 1; eventType = newPullParser.next()) {
                ...
                if (eventType == 2) {
                    if (this.xmlTagManager.getLocalNumberTag().isAvailable(name)) {
                        ...
                        String attributeValue = this.xmlTagManager.getChannelsIdTag().isAvailable(newPullParser.getAttributeName(0)) ? newPullParser.getAttributeValue(null, this.xmlTagManager.getChannelsIdTag().getTag()) : "";
                        Log.d(TAG, "Channels id: " + attributeValue);
                        XmlTagValid.checkInvalid(this.xmlTagManager.getChannelsIdTag().getTag(), attributeValue);
                        InsertChannels insertChannels = new InsertChannels();
                        insertConfig.getInsertChannels().put(Integer.parseInt(attributeValue), insertChannels);
                        parseChannel(newPullParser, insertChannels);
                    }
                } else if (eventType == 3 && this.xmlTagManager.getConfigTag().isAvailable(name)) {
        ...
        while (eventType != 1) {
            ...
            if (eventType == 2) {
                if (this.xmlTagManager.getChannelTag().isAvailable(name)) {
                    ...
                    String attributeValue = this.xmlTagManager.getChannelNameTag().isAvailable(xmlPullParser.getAttributeName(0)) ? xmlPullParser.getAttributeValue(null, this.xmlTagManager.getChannelNameTag().getTag()) : "";
