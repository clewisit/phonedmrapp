...
public class YModemManager {
    ...
    private static final String EXTERNAL_FILE_PATH = (SourceScheme.FILE.getUriPrefix() + EXTERNAL_PATH_NAME);
    ...
    public static ArraySet<Integer> mCanSendClientWhenOnDataReadySet;
    /* access modifiers changed from: private */
    public Set<Messenger> mClientMessengers = Collections.synchronizedSet(new ArraySet());
    private Context mContext;
    private YModem mYModem;
    /* access modifiers changed from: private */
    public YModemThread mYModemThread;
    ...
    public void startUpdateFirmware() {
        ...
        if (this.mYModem == null) {
            YModem build = new YModem.Builder().with(this.mContext).filePath(getDmrFirmwarePath()).fileName(getDmrFirmwareName()).sendSize(1024).callback(new YModemListener() {
    ...
    public void releaseYModem() {
        YModemThread yModemThread = this.mYModemThread;
        if (yModemThread != null) {
            yModemThread.setYModem((YModem) null);
