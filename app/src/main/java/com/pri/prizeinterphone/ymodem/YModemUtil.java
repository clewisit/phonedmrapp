...
class YModemUtil {
    ...
    static byte[] getFileNamePackage(String str, int i, String str2) throws IOException {
        byte[] bArr;
        byte[] bytes = (i + "").getBytes();
        byte[] concat = concat(str.getBytes(), new byte[]{0}, bytes);
        if (TextUtils.isEmpty(str2)) {
            bArr = Arrays.copyOf(concatByteArrays(concat, new byte[]{0}), 128);
        } else {
            bArr = Arrays.copyOf(concat(concat, new byte[]{0}, str2.getBytes()), 128);
        }
        return getDataPackage(bArr, 128, (byte) 0);
    }

    static byte[] getDataPackage(byte[] bArr, int i, byte b) throws IOException {
        byte[] dataHeader = getDataHeader(b, bArr.length == YModem.mSize.intValue() ? (byte) 2 : 1);
        if (i < bArr.length) {
            while (i < bArr.length) {
                bArr[i] = CPMEOF;
                i++;
        ...
        dataOutputStream.writeShort((short) ((int) crc16.calcCRC(bArr)));
    ...
    static InputStream getInputStream(Context context, String str) throws IOException {
