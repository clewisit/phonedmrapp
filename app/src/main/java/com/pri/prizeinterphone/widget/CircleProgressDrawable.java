...
public class CircleProgressDrawable extends View {
    ...
    public void onMeasure(int i, int i2) {
        ...
        int strokeWidth = (int) (((float) (measuredWidth > measuredHeight ? measuredHeight : measuredWidth)) - (this.mBackPaint.getStrokeWidth() > this.mProgPaint.getStrokeWidth() ? this.mBackPaint : this.mProgPaint).getStrokeWidth());
        int paddingLeft = getPaddingLeft() + ((measuredWidth - strokeWidth) / 2);
        int paddingTop = getPaddingTop() + ((measuredHeight - strokeWidth) / 2);
        this.mRectF = new RectF((float) paddingLeft, (float) paddingTop, (float) (paddingLeft + strokeWidth), (float) (paddingTop + strokeWidth));
        int[] iArr = this.mColorArray;
        if (iArr != null && iArr.length > 1) {
            this.mProgPaint.setShader(new LinearGradient(0.0f, 0.0f, 0.0f, (float) getMeasuredWidth(), this.mColorArray, (float[]) null, Shader.TileMode.MIRROR));
        }
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(this.mRectF, 275.0f, (float) ((this.mProgress * 360) / 100), false, this.mProgPaint);
    }
    ...
    public void setProgress(int i, long j) {
        ...
        ValueAnimator ofInt = ValueAnimator.ofInt(new int[]{this.mProgress, i});
    ...
    public void setBackWidth(int i) {
        this.mBackPaint.setStrokeWidth((float) i);
        invalidate();
    }
