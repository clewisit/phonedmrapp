...
public class MyNotificationManager {
    ...
    /* loaded from: classes4.dex */
    public static class MyNotificationManagerHolder {
        private static final MyNotificationManager INSTANCE = new MyNotificationManager(PrizeInterPhoneApp.getContext());
    ...
    public NotificationCompat.Builder getCommonNotificationBuilder(String str) {
        ...
        channelId.setContentText(str).setWhen(System.currentTimeMillis()).setSmallIcon(R.drawable.interphone_ic_title_icon).build();
        return channelId;
    }
    ...
    public void notifyUpdate2Notification(int i, int i2) {
        ...
        if (i == 32) {
            ongoing = getCommonNotificationBuilder(this.mContext.getString(R.string.interphone_update_firmware_content3)).setContentIntent(this.mStartHomeIntent).setOngoing(false);
        } else if (i == 64) {
            ongoing = getCommonNotificationBuilder(this.mContext.getString(R.string.interphone_update_firmware_content4)).setContentIntent(this.mStartUpdateFirmwareIntent).setOngoing(false);
        } else {
            ongoing = getCommonNotificationBuilder(this.mContext.getString(R.string.interphone_update_firmware_content5, Integer.valueOf(i2))).setContentIntent(this.mStartUpdateFirmwareIntent).setProgress(100, i2, false).setOngoing(true);
        }
