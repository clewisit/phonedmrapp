...
public class InterPhoneHomeActivity extends AppCompatActivity implements DmrManager.UpdateChannelDataNotificationListener, MessageLisenter, DmrManager.UpdateListener, CmdStateMachine.InitializedFeedBack {
    ...
    private LinearLayout mLlInterPhoneTapView;
    ...
    private DmrManager.MessageListener mGlobalSetChannelMessageListener = new DmrManager.MessageListener() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.1
        ...
        public void errorEvent(Byte b) {
            if (b.byteValue() == 34 || b.byteValue() == 35) {
                Snackbar.make(InterPhoneHomeActivity.this.mLlInterPhoneTapView, (int) R.string.operate_set_channel_fail_and_restart, -2).show();
                DmrManager.getInstance().unregisterEventListener(Byte.valueOf((byte) Const.Command.SET_DIGITAL_INFO_CMD), InterPhoneHomeActivity.this.mGlobalSetChannelMessageListener);
                DmrManager.getInstance().unregisterEventListener(Byte.valueOf((byte) Const.Command.SET_ANALOG_INFO_CMD), InterPhoneHomeActivity.this.mGlobalSetChannelMessageListener);
            }
        }
    ...
    public Runnable mDismissRunnable = new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.2
        @Override // java.lang.Runnable
        public void run() {
            InterPhoneHomeActivity.this.dismissProgressDialog();
        }
    };
    private Runnable mToastInitFail = new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.3
        @Override // java.lang.Runnable
        public void run() {
    ...
    private Runnable mToastInitTry = new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.4
        @Override // java.lang.Runnable
        public void run() {
            CmdStateMachine cmdStateMachine = CmdStateMachine.getInstance();
            cmdStateMachine.sendMessage(cmdStateMachine.obtainMessage(1));
        }
    };
    private Runnable mModulePowerOn = new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.5
        @Override // java.lang.Runnable
        public void run() {
            InterPhoneHomeActivity.this.init();
        }
    };
    private Runnable mStartUpdateActivity = new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.6
        @Override // java.lang.Runnable
        public void run() {
            InterPhoneHomeActivity.this.mHandler.post(InterPhoneHomeActivity.this.mDismissRunnable);
    ...
    @Override // com.pri.prizeinterphone.manager.MessageLisenter
    public void onMessageSend(MessageData messageData) {
    }

    @Override // com.pri.prizeinterphone.manager.DmrManager.UpdateChannelDataNotificationListener
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (YModemManager.getInstance().isRunning() || !Util.isDmrUpdateIdle() || YModemManager.getInstance().isExternalSdcardHaveFirmware()) {
            ...
            this.mHandler.postDelayed(this.mModulePowerOn, 1000L);
            this.mHandler.postDelayed(this.mToastInitTry, 6000L);
            this.mHandler.postDelayed(this.mToastInitFail, 10000L);
        }
        } catch (Throwable e) {
            // System API errors - this app requires system privileges
            android.util.Log.e("InterPhoneHomeActivity", "System API not available", e);
            // Toast disabled - app works fine without system APIs
        }
    ...
    private boolean checkHasPermissions() {
        for (String str : this.dangrousPermissions) {
    ...
    public void onDestroy() {
        Log.d(TAG, "onDestroy");
        CmdStateMachine.getInstance().setInitializedFeedBack(null);
        CmdStateMachine.getInstance().quitCmdMachine();
        this.mHandler.removeCallbacksAndMessages(null);
        DmrManager.getInstance().unregisterEventListener(Byte.valueOf((byte) Const.Command.SET_DIGITAL_INFO_CMD), this.mGlobalSetChannelMessageListener);
        DmrManager.getInstance().unregisterEventListener(Byte.valueOf((byte) Const.Command.SET_ANALOG_INFO_CMD), this.mGlobalSetChannelMessageListener);
        DmrManager.getInstance().unregisterUpdateListener(this);
        DmrManager.getInstance().removeMessageListener(this);
    ...
    public void tapOnClick(View view) {
        int i;
        switch (view.getId()) {
            case R.id.channel /* 2131296358 */:
                i = 1;
                break;
            case R.id.contacts /* 2131296377 */:
                i = 2;
                break;
            case R.id.local /* 2131296620 */:
                i = 4;
                break;
    ...
    public void updateModuleInit() {
        ...
        runOnUiThread(new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                InterPhoneHomeActivity.this.lambda$updateModuleInit$0();
            }
        });
    }
    ...
    @Override // com.pri.prizeinterphone.state.CmdStateMachine.InitializedFeedBack
    ...
    public void updateUnreadDot() {
        this.mHandler.post(new Runnable() { // from class: com.pri.prizeinterphone.InterPhoneHomeActivity.7
            @Override // java.lang.Runnable
            public void run() {
    ...
    private void setDefaultDisplayWhenDensityAndFontChange(Configuration configuration) {
        if (configuration.densityDpi == getDefaultDisplayDensity(0) && configuration.fontScale == 1.0d) {
            return;
        }
