...
public class DtmfTransport implements TransportProtocol {
    ...
    public void sendMessages(Set<Communicator.Message> set) {
        for (Communicator.Message messageDigits : set) {
            char[] messageDigits2 = getMessageDigits(messageDigits);
            if (messageDigits2 != null) {
                Log.i(this, "sendMessages: queueing message: %s", new Object[]{String.valueOf(messageDigits2)});
                this.mPendingMessages.offer(messageDigits2);
            }
        }
        ...
        synchronized (this.mDigitSendLock) {
            if (this.mMessageToSend == null && this.mDigitSendScheduledFuture == null) {
                ...
                if (poll != null) {
                    ...
                    this.mDigitSendScheduledFuture = this.mScheduledExecutorService.scheduleAtFixedRate(new DtmfTransport$$ExternalSyntheticLambda0(this), this.mDurationOfDtmfMessageMillis + getDtmfDurationFuzzMillis(), this.mIntervalBetweenDigitsMillis, TimeUnit.MILLISECONDS);
                }
            }
        }
    }
    ...
    private void handleProbeMessage() {
        ...
        if (!stringBuffer.startsWith(String.valueOf(DTMF_MESSAGE_START)) || !stringBuffer.endsWith(String.valueOf(DTMF_MESSAGE_DELIMITER)) || stringBuffer.length() <= 2) {
    ...
    private void scheduleNegotiationTimeout() {
        synchronized (this.mNegotiationLock) {
            this.mNegotiationFuture = this.mScheduledExecutorService.schedule(new DtmfTransport$$ExternalSyntheticLambda1(this), this.mNegotiationTimeoutMillis, TimeUnit.MILLISECONDS);
        }
    }
    ...
    private void scheduleDtmfMessageTimeout() {
        synchronized (this.mDtmfMessageTimeoutLock) {
            maybeCancelDtmfMessageTimeout();
            this.mDtmfMessageTimeoutFuture = this.mScheduledExecutorService.schedule(new DtmfTransport$$ExternalSyntheticLambda2(this), this.mDurationOfDtmfMessageMillis, TimeUnit.MILLISECONDS);
        }
    }
    ...
    public char[] getMessageDigits(Communicator.Message message) {
        Pair key = DTMF_TO_MESSAGE.getKey(message);
        if (key == null) {
            return null;
        }
