...
public class CarrierSmsUtils {
    ...
    public static String getImsRcsPackageForIntent(Context context, Phone phone, Intent intent) {
        ...
        for (ResolveInfo next : context.getPackageManager().queryIntentServices(intent, 0)) {
            ...
            if (serviceInfo == null) {
                String str = TAG;
                Rlog.e(str, "Can't get service information from " + next);
    ...
    private static String getImsRcsPackage(Phone phone) {
        ImsResolver instance = ImsResolver.getInstance();
        if (instance == null) {
        ...
        try {
            return instance.getConfiguredImsServicePackageName(phone.getPhoneId(), 2);
        } finally {
