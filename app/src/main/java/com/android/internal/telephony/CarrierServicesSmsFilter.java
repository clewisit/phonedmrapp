...
public class CarrierServicesSmsFilter {
    ...
    public static final UUID sAnomalyNoResponseFromCarrierMessagingService = UUID.fromString("94095e8e-b516-4065-a8be-e05b84071002");
    /* access modifiers changed from: private */
    ...
    public boolean filter() {
        ...
        if (this.mFilterAggregator == null) {
            ...
            callbackTimeoutHandler.sendMessageDelayed(callbackTimeoutHandler.obtainMessage(1, filterAggregator), 600000);
            for (String filterWithPackage : arrayList) {
                filterWithPackage(filterWithPackage, this.mFilterAggregator);
            }
            return true;
        }
    ...
    private List<String> getSystemAppForIntent(Intent intent) {
        ...
        for (ResolveInfo next : packageManager.queryIntentServices(intent, 0)) {
            ServiceInfo serviceInfo = next.serviceInfo;
            if (serviceInfo == null) {
                loge("getSystemAppForIntent: Can't get service information from " + next);
            } else {
    ...
    private final class CarrierSmsFilter {
        ...
        public void filterSms(CarrierSmsFilterCallback carrierSmsFilterCallback) {
            this.mSmsFilterCallback = carrierSmsFilterCallback;
            if (!this.mCarrierMessagingServiceWrapper.bindToCarrierMessagingService(CarrierServicesSmsFilter.this.mContext, this.mPackageName, new CarrierServicesSmsFilter$CarrierSmsFilter$$ExternalSyntheticLambda1(), new CarrierServicesSmsFilter$CarrierSmsFilter$$ExternalSyntheticLambda2(this))) {
        ...
        public void lambda$filterSms$1() {
            try {
                ...
                this.mCarrierMessagingServiceWrapper.receiveSms(new MessagePdu(Arrays.asList(this.mPdus)), this.mSmsFormat, this.mDestPort, CarrierServicesSmsFilter.this.mPhone.getSubId(), new CarrierServicesSmsFilter$CarrierSmsFilter$$ExternalSyntheticLambda0(), this.mSmsFilterCallback);
    ...
    private final class CarrierSmsFilterCallback implements CarrierMessagingServiceWrapper.CarrierMessagingCallback {
    ...
    private final class FilterAggregator {
        ...
        /* access modifiers changed from: package-private */
        public void onFilterComplete(int i, CarrierSmsFilterCallback carrierSmsFilterCallback) {
    ...
    protected final class CallbackTimeoutHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            if (message.what == 1) {
                ...
                UUID r0 = CarrierServicesSmsFilter.sAnomalyNoResponseFromCarrierMessagingService;
                AnomalyReporter.reportAnomaly(r0, "No response from " + ((String) ((FilterAggregator) message.obj).mCallbacks.stream().map(new CarrierServicesSmsFilter$CallbackTimeoutHandler$$ExternalSyntheticLambda0()).collect(Collectors.joining(", "))), CarrierServicesSmsFilter.this.mPhone.getCarrierId());
                handleFilterCallbacksTimeout();
            }
        }

        private void handleFilterCallbacksTimeout() {
            for (CarrierSmsFilterCallback onReceiveSmsComplete : CarrierServicesSmsFilter.this.mFilterAggregator.mCallbacks) {
                CarrierServicesSmsFilter.this.log("handleFilterCallbacksTimeout: calling onFilterComplete");
                onReceiveSmsComplete.onReceiveSmsComplete(0);
            }
        }
    }
}
