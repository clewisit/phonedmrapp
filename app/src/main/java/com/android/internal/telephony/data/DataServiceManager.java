...
public class DataServiceManager extends Handler {
    ...
    /* access modifiers changed from: private */
    public List<DataCallResponse> mLastDataCallResponseList = Collections.EMPTY_LIST;
    /* access modifiers changed from: private */
    ...
    private class DataServiceManagerDeathRecipient implements IBinder.DeathRecipient {
        ...
        public void binderDied() {
            ...
            for (Message r1 : DataServiceManager.this.mMessageMap.values()) {
                DataServiceManager.this.sendCompleteMessage(r1, 4);
            }
    ...
    private void grantPermissionsToService(String str) {
        ...
        try {
            this.mPermissionManager.grantDefaultPermissionsToEnabledTelephonyDataServices(strArr, UserHandle.of(UserHandle.myUserId()), new DataServiceManager$$ExternalSyntheticLambda0(), new DataServiceManager$$ExternalSyntheticLambda2(this, countDownLatch));
    ...
    private final class DataServiceCallbackWrapper extends IDataServiceCallback.Stub {
        ...
        public void onRequestDataCallListComplete(int i, List<DataCallResponse> list) {
            ...
            Message message = (Message) DataServiceManager.this.mMessageMap.remove(asBinder());
            if (message != null) {
                message.getData().putParcelableList(DataServiceManager.DATA_CALL_RESPONSE, list);
            }
            DataServiceManager.this.sendCompleteMessage(message, i);
            if (DataServiceManager.this.mTransportType == 1) {
                if (DataServiceManager.this.mLastDataCallResponseList.size() != list.size() || !DataServiceManager.this.mLastDataCallResponseList.containsAll(list)) {
                    ...
                    DataServiceManager.this.loge(str);
                    if (!((Set) list.stream().map(new DataServiceManager$DataServiceCallbackWrapper$$ExternalSyntheticLambda0()).collect(Collectors.toSet())).equals(DataServiceManager.this.mLastDataCallResponseList.stream().map(new DataServiceManager$DataServiceCallbackWrapper$$ExternalSyntheticLambda0()).collect(Collectors.toSet()))) {
        ...
        public void onDataCallListChanged(List<DataCallResponse> list) {
            ...
            dataServiceManager.mLastDataCallResponseList = list2;
            DataServiceManager.this.mDataCallListChangedRegistrants.notifyRegistrants(new AsyncResult((Object) null, list, (Throwable) null));
        }
    ...
    public void setupDataCall(int i, DataProfile dataProfile, boolean z, boolean z2, int i2, LinkProperties linkProperties, int i3, NetworkSliceInfo networkSliceInfo, TrafficDescriptor trafficDescriptor, boolean z3, Message message) {
        ...
        if (message2 != null) {
            this.mMessageMap.put(dataServiceCallbackWrapper.asBinder(), message2);
        }
        try {
            sendMessageDelayed(obtainMessage(2, dataServiceCallbackWrapper), REQUEST_UNRESPONDED_TIMEOUT);
    ...
    public void startHandover(int i, Message message) {
        DataServiceCallbackWrapper dataServiceCallbackWrapper = setupCallbackHelper("startHandover", message);
        if (dataServiceCallbackWrapper == null) {
    ...
    public void cancelHandover(int i, Message message) {
        DataServiceCallbackWrapper dataServiceCallbackWrapper = setupCallbackHelper("cancelHandover", message);
        if (dataServiceCallbackWrapper == null) {
            sendCompleteMessage(message, 4);
            return;
        }
    ...
    private DataServiceCallbackWrapper setupCallbackHelper(String str, Message message) {
        log(str);
        if (!this.mBound) {
            sendCompleteMessage(message, 4);
            return null;
        }
        DataServiceCallbackWrapper dataServiceCallbackWrapper = new DataServiceCallbackWrapper(str);
        if (message != null) {
        ...
        return dataServiceCallbackWrapper;
    }
