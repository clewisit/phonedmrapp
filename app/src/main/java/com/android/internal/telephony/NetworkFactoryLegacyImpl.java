...
class NetworkFactoryLegacyImpl extends Handler implements NetworkFactoryShim {
    ...
    private final Map<NetworkRequest, NetworkRequestInfo> mNetworkRequests = new LinkedHashMap();
    ...
    public void evalRequests() {
        for (NetworkRequestInfo evalRequest : this.mNetworkRequests.values()) {
            evalRequest(evalRequest);
        }
    }

    public void reevaluateAllRequests() {
        post(new NetworkFactoryLegacyImpl$$ExternalSyntheticLambda0(this));
    }

    public void releaseRequestAsUnfulfillableByAnyFactory(NetworkRequest networkRequest) {
        post(new NetworkFactoryLegacyImpl$$ExternalSyntheticLambda1(this, networkRequest));
    }
