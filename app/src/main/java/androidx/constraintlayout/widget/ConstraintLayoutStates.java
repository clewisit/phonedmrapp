...
public class ConstraintLayoutStates {
    ...
    private void parseConstraintSet(Context context, XmlPullParser xmlPullParser) {
        ...
        for (int i2 = 0; i2 < attributeCount; i2++) {
            if (XmlTagConst.TAG_CHANNELS_ID.equals(xmlPullParser.getAttributeName(i2))) {
                String attributeValue = xmlPullParser.getAttributeValue(i2);
                if (attributeValue.contains("/")) {
                    i = context.getResources().getIdentifier(attributeValue.substring(attributeValue.indexOf(47) + 1), XmlTagConst.TAG_CHANNELS_ID, context.getPackageName());
                } else {
                    i = -1;
                }
                if (i == -1) {
